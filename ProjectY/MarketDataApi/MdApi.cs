...
namespace MarketDataApi
{
    public class MdApi
    {
        ...
        private RequestSocket _socketReq;
        public MdApi(string subIP, int subPort, string reqIP, int reqPost)
        {
            if (string.IsNullOrEmpty(subIP) == false)
                BuildSubSocket(subIP, subPort);
            if (string.IsNullOrEmpty(reqIP) == false)
                BuildReqSocket(reqIP, reqPost);
        }

        private void BuildReqSocket(string ip, int port)
        {
            _socketReq = new RequestSocket(string.Format(">tcp://{0}:{1}", ip, port));
        }

        public List<object> GetContracts(AdapterCode adapterCode)
        {
            var output = new List<object>();
            bool isLast = false;
            while (isLast == false)
            {
                output.AddRange(GetContracts(adapterCode, output.Count, 200, out isLast));
            }
            return output;
        }

        private List<object> GetContracts(AdapterCode adapterCode,int startIndex, int increment, out bool isLast)
        {
            var output = new List<object>();
            string prefix;
            switch (adapterCode)
            {
                case AdapterCode.TSE:
                case AdapterCode.TPEX:    
                    prefix = string.Format("{0}#{1}", (int)adapterCode, "1");
                    break;
                case AdapterCode.TAIFEX_FUTURES_DAY:
                case AdapterCode.TAIFEX_FUTURES_NIGHT:
                    prefix = string.Format("{0}#{1}", (int)adapterCode, "11");
                    break;                
                case AdapterCode.TAIFEX_OPTIONS_DAY:
                case AdapterCode.TAIFEX_OPTIONS_NIGHT:
                    prefix = string.Format("{0}#{1}", (int)adapterCode, "41");
                    break;
                case AdapterCode.GLOBAL_PATS:
                default:
                    isLast = true;
                    return output;
            }
            _socketReq.SendMoreFrame("GetContracts").SendFrame(string.Format("{0},{1},{2}", prefix, startIndex, increment));
            var result = _socketReq.ReceiveMultipartBytes();
            if (result.Count == 0)
            {
                isLast = true;
                return output;
            ...
            {
                var lastByteArray = result[result.Count - 1];
                if (lastByteArray.Length != 1)
                {
                    isLast = true;
                    return output;
                }
                ...
                {
                    isLast = lastByteArray[0] == 0 ? false : true;
                    for (int i = 0; i < result.Count - 1; i++)
                    {
                        switch (adapterCode)
                        {
                            case AdapterCode.TSE:
                                output.Add(new PacketTSE.Format1(result[i]));
                                break;
                            case AdapterCode.TPEX:
                                output.Add(new PacketTPEX.Format1(result[i]));
                                break;
                            case AdapterCode.TAIFEX_FUTURES_DAY:
                            case AdapterCode.TAIFEX_FUTURES_NIGHT:
                            case AdapterCode.TAIFEX_OPTIONS_DAY:
                            case AdapterCode.TAIFEX_OPTIONS_NIGHT:
                                output.Add(new PacketTAIFEX.I010(result[i], 0));                                
                                break;
                            case AdapterCode.GLOBAL_PATS:
                                break;
                            default:
        ...
        public object GetSnapshot(AdapterCode adapterCode, string packetType, string packetTypeSection)
        {
            var sysPacketType = GetPacketType(adapterCode, packetType);
            _socketReq.SendMoreFrame("GetSnapshot").SendFrame(string.Format("{0}#{1},{2}", (int)adapterCode, sysPacketType, packetTypeSection));
            var result = _socketReq.ReceiveMultipartBytes();
            if (result.Count == 1)
            {
                ...
                {
                    case AdapterCode.TSE:
                        switch (sysPacketType)
                        {
                            //case "1":
                            //    return new PacketTSE.Format1(result[0]);
                            case "6":
                                return new PacketTSE.Format6(result[0]);
                            case "17":
                                return new PacketTSE.Format17(result[0]);
                            default:
                                return null;
                        }
                    case AdapterCode.TPEX:
                        ...
                        {
                            //case "1":
                            //    return new PacketTPEX.Format1(result[0]);
                            case "6":
                                return new PacketTPEX.Format6(result[0]);
                            case "17":
                                return new PacketTPEX.Format17(result[0]);
                            default:
                                return null;
                        }
                    case AdapterCode.TAIFEX_FUTURES_DAY:                    
                    case AdapterCode.TAIFEX_FUTURES_NIGHT:                        
                        ...
                        {
                            //case "11":
                            //    return new PacketTAIFEX.I010(result[0], 0);
                            case "21":
                                return new PacketTAIFEX.I020(result[0], 0);
                            case "22":
                                return new PacketTAIFEX.I080(result[0], 0);
                            case "27":
                                return new PacketTAIFEX.I022(result[0], 0);
                            case "28":
                                return new PacketTAIFEX.I082(result[0], 0);
                            default:
                                return null;
                        }
                    case AdapterCode.TAIFEX_OPTIONS_DAY:
                    ...
                    case AdapterCode.GLOBAL_PATS:
                        ...
                        {
                            //case "0":
                            //    return new PacketPATS.Format0(result[0]);
                            case "1":
                                return new PacketPATS.Format1(result[0], packetTypeSection);
                            case "2":
                                return new PacketPATS.Format2(result[0], packetTypeSection);
                            default:
                                return null;
                        }
                    default:
                                ...
                                switch (data.Length)
                                {
                                    case 3:
                                        switch (data[0])
                                        {
                                            ...
                                            case "6":
                                                OnUnSolvedPacketReceived(AdapterCode.GLOBAL_PATS, data[1], messages[1]); break;
                                            default:
                                                break;
                                        }
                                        break;
        ...
        private void ProcessSolvedPacket_TSE(string packetType, byte[] packetData)
        {
            try
            {
                ...
                {
                    case "1":
                        OnTseFormat1Received(new PacketTSE.Format1(packetData));
                        break;
                    case "6":
        ...
        private void ProcessSolvedPacket_TPEX(string packetType, byte[] packetData)
        {
            try
            {
                ...
                {
                    case "1":
                        OnTpexFormat1Received(new PacketTPEX.Format1(packetData));
                        break;
                    case "6":
        ...
        private void ProcessSolvedPacket_TAIFEX(string packetType, byte[] packetData)
        {
            try
            {
                ...
                {
                    case "I010":
                        OnTaifexI010Received(new PacketTAIFEX.I010(packetData, 0));
                        break;
                    case "I020":
                        OnTaifexI020Received(new PacketTAIFEX.I020(packetData, 0));
                        break;
                    case "I080":
                        OnTaifexI080Received(new PacketTAIFEX.I080(packetData, 0));
                        break;
                    case "I022":
                        OnTaifexI022Received(new PacketTAIFEX.I022(packetData, 0));
                        break;
                    case "I082":
                        OnTaifexI082Received(new PacketTAIFEX.I082(packetData, 0));
                        break;
                    default:
                        break;
                }
        ...
        private void ProcessSolvedPacket_PATS(string packetType, byte[] packetData, string symbol)
        {
            try
            {
                ...
                {
                    case "0":
                        OnPatsFormat0Received(new PacketPATS.Format0(packetData)); break;
                    case "1":
                        OnPatsFormat1Received(new PacketPATS.Format1(packetData, symbol)); break;
                    case "2":
                        OnPatsFormat2Received(new PacketPATS.Format2(packetData, symbol)); break;
                    default:
                        break;
                }
            }
        ...
        {
            public readonly PacketTAIFEX.I082 PacketData;
            public TaifexI082ReceivedEventArgs(PacketTAIFEX.I082 packetData)
            {
                PacketData = packetData;
            }
        }
        private void OnTaifexI082Received(PacketTAIFEX.I082 packetData)
        {
        ...
        {
            public readonly PacketTAIFEX.I080 PacketData;
            public TaifexI080ReceivedEventArgs(PacketTAIFEX.I080 packetData)
            {
                PacketData = packetData;
            }
        }
        private void OnTaifexI080Received(PacketTAIFEX.I080 packetData)
        {
        ...
        {
            public readonly PacketTAIFEX.I022 PacketData;
            public TaifexI022ReceivedEventArgs(PacketTAIFEX.I022 packetData)
            {
                PacketData = packetData;
            }
        }
        private void OnTaifexI022Received(PacketTAIFEX.I022 packetData)
        {
        ...
        {
            public readonly PacketTAIFEX.I010 PacketData;
            public TaifexI010ReceivedEventArgs(PacketTAIFEX.I010 packetData)
            {
                PacketData = packetData;
            }
        }
        private void OnTaifexI010Received(PacketTAIFEX.I010 packetData)
        {
        ...
        {
            public readonly PacketTSE.Format1 PacketData;
            public TseFormat1ReceivedEventArgs(PacketTSE.Format1 packetData)
            {
                PacketData = packetData;
            }
        }
        private void OnTseFormat1Received(PacketTSE.Format1 packetData)
        {
        ...
        {
            public readonly PacketTPEX.Format1 PacketData;
            public TpexFormat1ReceivedEventArgs(PacketTPEX.Format1 packetData)
            {
                PacketData = packetData;
            }
        }
        private void OnTpexFormat1Received(PacketTPEX.Format1 packetData)
        {
        ...
        {
            public readonly PacketTAIFEX.I020 PacketData;
            public TaifexI020ReceivedEventArgs(PacketTAIFEX.I020 packetData)
            {
                PacketData = packetData;
            }
        }
        private void OnTaifexI020Received(PacketTAIFEX.I020 packetData)
        {
        ...
        {
            public readonly PacketTSE.Format6 PacketData;
            public TseFormat6ReceivedEventArgs(PacketTSE.Format6 packetData)
            {
                PacketData = packetData;
            }
        }
        private void OnTseFormat6Received(PacketTSE.Format6 packetData)
        {
        ...
        {
            public readonly PacketTPEX.Format6 PacketData;
            public TpexFormat6ReceivedEventArgs(PacketTPEX.Format6 packetData)
            {
                PacketData = packetData;
            }
        }
        private void OnTpexFormat6Received(PacketTPEX.Format6 packetData)
        {
        ...
        {
            public readonly PacketPATS.Format0 PacketData;
            public PatsFormat0ReceivedEventArgs(PacketPATS.Format0 packetData)
            {
                PacketData = packetData;
            }
        }
        private void OnPatsFormat0Received(PacketPATS.Format0 packetData)
        {
        ...
        {
            public readonly PacketPATS.Format1 PacketData;
            public PatsFormat1ReceivedEventArgs(PacketPATS.Format1 packetData)
            {
                PacketData = packetData;
            }
        }
        private void OnPatsFormat1Received(PacketPATS.Format1 packetData)
        {
        ...
        {
            public readonly PacketPATS.Format2 PacketData;
            public PatsFormat2ReceivedEventArgs(PacketPATS.Format2 packetData)
            {
                PacketData = packetData;
            }
        }
        private void OnPatsFormat2Received(PacketPATS.Format2 packetData)
        {
