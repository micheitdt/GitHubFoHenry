    ...
    public class Format1
    {
        ...
        public Format1(byte[] data)
        {
            try
            {
                ...
                {
                    _logger.Info(string.Format("TSE_Format1(): datalength = {0} needlength = {1}  Data={2}.", data.Length, Header.Length -3, Encoding.Default.GetString(data)));
                    return;
                }
                StockID = Encoding.ASCII.GetString(data, 10, 6).Trim();
                StockName = Encoding.ASCII.GetString(data,16,16).Trim();
            ...
            {
                _logger.Error(err, string.Format("TSE_Format1(): ErrMsg = {0} Data={1}.", err.Message, Encoding.Default.GetString(data)));
            }
        }
    }
}
